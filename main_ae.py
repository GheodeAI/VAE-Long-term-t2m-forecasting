...
for city in cities:
    ...
    for iter_count in range(10):
        ...
        for delay in range(1, 5):
            """Employ the second-stage AE part.
            ...
            latent_space_train = model_1.predict(x_train)

            # print(city[0] + '  ' + str(x_test.shape)) for testing purposes
            # Remove the previous model to avoid any troubles
            try:
                del model_2
            except Exception:
                pass
            ...
            """
            x_test = x_test[:, 0]
            output_test = model_1.predict(x_test)  # model_1.autoencoder.predict(x_test)
            t2m_pred = model_2.autoencoder.predict(output_test)
            ...
            # Result's graphs - tabular
            (climate, ground_truth) = prepare_data.prepare_data(['t2m'], preprocessing=['none'],
                                                                border=year_of_pred)
            ...
            with open(city[0] + '_stats_' + str(iter_count) + 'AE_' + str(AE) + '_AE_' + str(timer) + '_delay_' + str(delay) + '.txt', 'w') as f:
                ...
                f.write('\n**********\n')
                f.write('\npersistence\n')
                f.write(str(statistical_analysis(ground_truth, persistence)))
                f.write('\nclimate\n')
                f.write(str(statistical_analysis(ground_truth, climate)))
                f.write('\nt2m\n')
                f.write(str(stats))
                f.write('\n**********\n')
